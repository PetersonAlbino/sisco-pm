...
$(document).ready(function(){
	...
	$("#home,#apps,#blog,#aboutus,#support,#contact").mouseover(function(){ 
	$("#sprite").stop().animate({backgroundPosition:''+items[$(this).attr("id")]+'px '+height+'px'},{duration:320});});
	$("#home,#apps,#blog,#aboutus,#support,#contact").mouseout(function(){ if(lock == 0) { $("#sprite").stop().animate({backgroundPosition:''+items[$('.selected').attr("id")]+'px '+height+'px'},{duration:320}); } });
	$("#home,#apps,#blog,#aboutus,#support,#contact").mousedown(function(){ lock =1; });
	...
	$(".syncopy_iphone_dot").mousedown(function(){
	 ...
	 {
	  swap_screenshot(current_screenshot*1+1, 1);  
	 } 
	  });
	 ...
	 if(1 != current_screenshot)
	 {
	  swap_screenshot(current_screenshot*1-1, 1);
	 	...
	 	$(".synotes_iphone_dot").mousedown(function(){
	 	 ...
	 	 if(total_screenshots != current_screenshot)
	 	 {
	 	  swap_screenshot(current_screenshot*1+1, 2);  
	 	 } 
	 	  });
	...
	var pt = $(this).attr('id') * 51 + 3;
	$("#feature_expanded_outer").stop().animate({marginTop:pt+'px'},{duration:320});
	
	
	
	//alert($( "."+$(this).attr('class').replace('syncopy_iphone_simple ', '')+" div" ).() ); 
...
function swap_screenshot(image, where)
{
	...
	else
	{
		...
		{
		$("#"+app+"_iphone_dot_"+current_screenshot).attr('class', app+'_iphone_dot');
		
		if(current_screenshot > image)
		{
 			$("#iphone_screenshots"+app_2).animate({ opacity: 0, marginTop: (margin-30)+"px", }, 300 )
 			.animate({ marginTop: (margin*1+30)+"px", }, 1 )
 			.animate({ backgroundPosition: "0 " + (image*size-size) + "px", }, 1 )
 			.animate({ opacity: 1, marginTop: margin+"px", }, 300 );
		} else
		{
		$("#iphone_screenshots"+app_2).animate({ opacity: 0, marginTop: (margin*1+30)+"px", }, 300 )
		...
		$("#"+app+"_iphone_dot_"+image).attr('class', app+'_iphone_dot '+app+'_iphone_dot_active');
		//syncopy_iphone_dot_active
		}	
}
